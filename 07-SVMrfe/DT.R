...
for (i in 20:116){
  ...
  sv.f <- svm(Tag~., data=n.test, cross=5, type='C-classification', kernel='radial')
  acc.svm3 = c(acc.svm3, sv.f$tot.accuracy)
  ...
  acc.kmm3 <- c(acc.kmm3,(tab[1,1]+tab[2,2]) / nrow(test.f))
}
# SVM plot
png('SVM after rfe rank.png')
plot(x=seq(20,116), y=acc.svm3, xlab='n from 20 to 116', ylab="Accuracy", main='SVM after rfe rank', 
     col='olivedrab')
dev.off()
# KNN plot
png('KNN after rfe rank.png')
plot(x=seq(20,116), y=acc.kmm3, xlab='n from 20 to 116', ylab="Accuracy", main='KNN after rfe rank', 
     col='orangered')
dev.off()

# 对比svm-Rfe及主成分分析模型
wilcox.test(acc.kmm.pca, acc.kmm3, paired = TRUE, alternative = "less")
wilcox.test(acc.svm.pca, acc.svm3, paired = TRUE, alternative = "less")
